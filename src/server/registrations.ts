...
const WORKFLOW_CONTRACT = {
  ...
  finalGate: {
    ...
    requiredOutcome:
      "No error-severity violations and no missing workflow/tool/component-source/token-resolution/decision evidence may remain before generated UI is accepted.",
  },
  harnessPolicy: {
    enforceableByServer: true,
    enforceableByCli: true,
    serverGuarantee:
      "The server records workflowSessionId-scoped tool calls in memory and validate_design_contract rejects final handoff when required MCP calls are missing from that audit trail.",
  },
  ci: {
